  ...
  viewProfileBtn.onclick = async () => {
    ...
    try {
      ...
      if (data.preference === "anonymous") {
        profileContent.innerHTML = `<div class="profile-view"><div style="font-size:56px; margin-bottom:10px;">👤</div><h2>Anonymous Profile</h2><p class="username-tag">@${data.username}</p><div class="profile-badge-row"><span>${badgeData.badge_icon}</span><span>${badgeData.badge_level}</span><span style="opacity:0.6;">· ${badgeData.badge_points} pts</span></div><div class="info-item" style="width:100%; text-align:center; margin-top:6px;"><div class="info-value" style="opacity:0.65; font-size:13px;">Your identity is hidden.</div></div></div>`;
        return;
      }
