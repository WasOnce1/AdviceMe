document.addEventListener("DOMContentLoaded", () => {
  ...
  showBtn.onclick = async () => {
    ...
    try {
      const res  = await fetch(`https://api.adviceme.social/api/requests/category/${encodeURIComponent(category)}`, { headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
      ...
      if (!data.length) {
        peopleSection.innerHTML = `<div class="empty-queue"><div class="eq-icon">🔍</div><p>No pending requests</p><span>There are currently no requests in the "${category}" category. Try another one.</span></div>`;
        return;
      }
      data.forEach(req => {
        ...
        div.innerHTML = `<div class="person-header"><div class="person-username">👤 ${req.username}</div><div class="person-urgency ${req.urgency || 'low'}">${urgencyLabel}</div></div><div class="person-message">${req.request_text}</div><div class="person-respond-hint"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Click to respond</div>`;
  ...
  removeBtn.onclick = async () => {
    ...
    await fetch(`https://api.adviceme.social/api/requests/remove/${activePerson.dataset.track}`, { method: "PUT", headers: { Authorization: `Bearer ${token}` } });
    activePerson.remove();
  ...
  respondBtn.onclick = async () => {
    ...
    if (!text) { showModalMsg("Please write your advice before sending.", "error"); return; }
    ...
    try {
      const res = await fetch("https://api.adviceme.social/api/advice/respond", {
        ...
        body: JSON.stringify({ track_id: activePerson.dataset.track, category: activePerson.dataset.category, advice_text: text })
      });
      if (!res.ok) { const d = await res.json(); showModalMsg(d.message || "Failed to send. Try again.", "error"); return; }
      showModalMsg("✅ Advice sent successfully!", "success");
      activePerson.remove();
      loadStats();
      setTimeout(() => { modal.classList.add("hidden"); activePerson = null; }, 1200);
    } catch {
  ...
  viewProfileBtn.onclick = async () => {
    ...
    try {
      ...
      if (data.preference === "anonymous") {
        profileContent.innerHTML = `<div class="profile-view"><div style="font-size:56px; margin-bottom:10px;">👤</div><h2>Anonymous Profile</h2><p class="username-tag">@${data.username}</p><div class="profile-badge-row"><span>${badgeData.badge_icon}</span><span>${badgeData.badge_level}</span><span style="opacity:0.6;">· ${badgeData.badge_points} pts</span></div><div class="info-item" style="width:100%; text-align:center; margin-top:6px;"><div class="info-value" style="opacity:0.65; font-size:13px;">Your identity is hidden.</div></div><button class="profile-logout-btn" id="logoutBtn" style="margin-top:14px; width:100%;">🚪 Logout</button></div>`;
  ...
  badgeBtn.onclick = async () => {
    ...
    try {
      ...
      const levelsHTML = allLevels.map(l => `<div class="badge-level-row ${l.label === currentLevel.level ? 'current' : ''}"><span>${l.icon}</span><span>${l.label}</span><span style="margin-left:auto; font-size:11px; opacity:0.5;">${l.min}+</span></div>`).join("");
      badgeValue.innerHTML = `<div class="badge-big-icon">${data.badge_icon}</div><div class="badge-level-name">${data.badge_level}</div><div class="badge-points-count">${data.badge_points} badge points</div><div class="badge-progress-wrap"><div class="badge-progress-label"><span>${prevMin} pts</span><span>${next ? next.min + ' pts' : 'Max'}</span></div><div class="badge-progress-bar"><div class="badge-progress-fill" style="width: ${progress}%"></div></div></div><div class="badge-next-info">${next ? `${next.min - data.badge_points} more points to reach ${next.label}` : '🏆 You have reached the maximum level!'}</div><div class="badge-levels-grid">${levelsHTML}</div>`;
    } catch {
