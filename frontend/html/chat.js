document.addEventListener("DOMContentLoaded", async () => {
  ...
  /* ================= FETCH MY USERNAME ================= */
  const meRes  = await fetch("http://localhost:3000/api/profile/me", {
  ...
  async function loadChat() {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/chat/${adviceId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  chatHeader.onclick = async (e) => {
    ...
    try {
      const profileRes = await fetch(`http://localhost:3000/api/profile/view/${otherUsername}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (data.user_type === 'giver') {
        try {
          const badgeRes  = await fetch(`http://localhost:3000/api/badge/user/${otherUsername}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
  ...
  removeRecipientBtn.onclick = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/chat/remove/${adviceId}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` }
      });
